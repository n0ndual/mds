...
* linux 网络体系结构学习
** bridge
相当于交换机，switch，linux主机本身就可用作交换机
** NAT
   配置一个支持NAT虚拟网络就相当于多加一个交换机
** linux 网络体系结构与 传统的网络体系结构的不同
*** 课本讲的是协议和体系结构，而linux的实现是另外一回事
*** 体系结构需要各种各样的硬件设备，而linux主机的功能可以与所有硬件设备等价
** 在linux中如何手动配置一个bridge
** 在linux中如何手动配置一个NAT
** overlay network的特点
** openvswitch与bridge有啥不同
* aws dcos template 地址
https://s3-external-1.amazonaws.com/cf-templates-1vm4xxu1los67-us-east-1/2015286IBP-template1
* 下一步工作计划
** 试用kubernetes的所有例子
** 试用mesos的所有例子
** 熟悉kuberetes的配置
...
** ssh -t clive@7.7.7.144 "echo '$USER ALL=NOPASSWD:ALL' | (sudo su -c 'EDITOR="tee" visudo -f /etc/sudoers')"
* chef 写到哪种程度
** 写recipe还是写shell script?
** 不理解chef的原理
** 不理解ruby语言和ruby项目
** 可能还得学习ruby才能会用chef和vagrant
** 实在不行，可以先用script安装kubernetes-on-mesos
* 还需要写一个client
* 如何provision一个linux box
** 直接使用vagrant package, 有一个bug
   要先把/home/vagrant/.ssh/authenrized_keys换成从vagrant官方下载的insecure key
* emacs跳转到最后一行
M+>
* immutable infrastructure
** setup operation system
*** create coreos image using packer
** setup DNS (hosts)
*** SkyDNS or DNSimple pre-installed
** install software
*** pre-installed
** set ssh no-password access
*** pre-installed
** start Mesos, Zookeeper, Kubernetes, Spark services
*** flannel using systemd
* kubernetes和mesos是不是完美互补
** 资源调度如果放入操作系统层，coreos或者kubernetes中更好，变成系统调用？
** 学习kubernetes架构和源码
** 学习mesos源码和架构
** 调研是否有另外的实现方式
* 是否使用coreos？
...
** object storage
* why you should NOT run hadoop on SAN or NAS
** Ceph/GlusterFs
*** Ceph as a scalable alternative to the Hadoop
** Lustre
*** Rutman N. Map/reduce on lustre[R]. white paper). Technical report, Xyratex Technology Limited, Havant, Hampshire, UK, 2
*** MapReduce over Lustre: Can RDMA-Based Approach Benefit?
*** Performability Comparison of Lustre and HDFS for MR Applications
** my point of view
*** data is distributed to thousands of nodes in HDFS, SAN or NAS does not have so many shards
*** multiple nodes contending for the same resources in SAN or NAS. BandWidth is shared.
*** to run hadoop on SAN or NAS, IB is a must, cost/performance is worse than hdfs
* object storage
** motivation
** architecture
...
** programming model does Not support data locality, use high storage bindwidth and high interconnet
** two character:
*** object storage
*** parallel file system
** why not NFS/NAS
...
*** In low end HPC storage environments, NFS can still be a very effective medium for distributing data, where low end HPC storage systems are defined as capacity under 100TB and high end generally above 1PB
** why not SAN
...
* Big data with GlusterFS
** glusterfs 支持绝大部分商用存储硬件，不关心硬件架构
** glusterfs 目前不支持data locality, hadoop 可以直接使用glusterfs但性能会差
** glusterfs 计划3.7版本支持hadoop的data locality
** 如果不使用infiniband,
** 目前将就着用
* performance evaluation
** MapReduce on hdfs, ceph, glusterfs, lustre, mapr with or without infiniband/RDMA
** weave, docker/network, kubernetes with or without rdma
* 下一步工作
** 资源监控，与lpd的页面集成
** 把MPI集成上去
** 把spark所依赖的存储环境配置好
** 部署优化
* 11.24 周报
** 上周五关于如何在物理机上部署的讨论之后，又发现了一个问题；
*** apache mesos 和mesosphere 不希望把mesos master 和slave 运行在容器中，但coreos的哲学是所有应用和服务都运行在容器中。dcos的做法是自己编译了一个包含所有依赖的二进制包。这个矛盾看起来很小，但细思极恐。资源调度是数据中心操作系统的核心，coreos也是一个云操作系统，必须把资源调度作为一个系统接口，只不过目前还没有实现。未来还有很多可能，目前还是用最简单的方案吧。
*** 借鉴dcos的做法，需要对每个依赖每次升级编译打包，太麻烦
*** 把mesos master和slave运行在容器中，有bug，缺少社区支持，不稳定
*** 目前的打算是试一下snappy ubuntu，自己打包一个镜像
** 读了几篇存储架构的论文，调研了存储市场，发现了一些研究空白
* 学习音乐
** 学习乐理，学习grageband
** 学习电钢琴
* IDC,gartner,cloud security alliance: 咨询公司
** IDC's worldwide big data taxonomy
** IDC's worldwide storage for big data and business analytics taxonomy
* 永生，机器人
** 一个人生目标：生存，探索真理
** 方法：理性，逻辑，归纳法
** 唯有成为一个机器人，才能对抗死亡
...
* 下一个月计划：
** 三周后报告dockletM可用了。
** 三周后读完osdi2014，sosp2015，思考自己的研究方向
** 一个月后读完集合论
** 读完音乐理论
* 最近的生活感悟
** 情感波动的时候，就写写日记，创作一些素材。为了排遣孤独感，必须进行创作：民谣，小说，音乐，情色
** 大脑疲倦的时候，就出去散散步，一天至少保证2个小时的运动，散步也算
** 早饭要吃，午饭晚饭少吃，不抽烟少喝酒。
*
